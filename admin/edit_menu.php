...
<div class="wrap">
	<div id="content">
		...
		<form method="post">
			...
			while($row=$res->fetch_array()){
			?>
			<input name="m_menu_name[]" value="<?php echo $row['m_menu_name']; ?>" placeholder="menu name :" /> <input name="m_menu_link[]" value="<?php echo $row['m_menu_link']; ?>" placeholder="menu link :" /> <input class="smaller" name="m_menu_order[]" value="<?php echo $row['m_menu_order']; ?>" placeholder="order :" /><input name="m_menu_id[]" type="hidden" value="<? echo $row['m_menu_id']; ?>"><button type="submit" name="delete_main_menu" value="<? echo $row['m_menu_id']; ?>">Delete</button><br />
			<?php
			}
			if(mysqli_num_rows($res)>0){
				echo '<button type="submit" name="edit_main_menu">Edit main menu</button>';
			}
			?>
